...
- Qwen uses its Transformers backend on one A10G instead of vLLM. The serverless endpoint has a one-container limit and can scale to zero to minimize GPU costs. After a scale-to-zero event, the next request takes approximately 20 seconds while Modal starts the container and loads the model into GPU memory. Subsequent requests use the warm container and are faster. The one-container limit also means that concurrent requests must wait.
