...
- The benchmark uses the current MERaLiON Part 4 rows from Hugging Face's dataset viewer API as its source of audio and reference transcripts. This removes the local manifest and keeps reproduction lightweight, but results depend on the current dataset state and the public viewer service.
...
- The benchmark runs all providers concurrently for each sample, while samples stay sequential. This reduces run time without overlapping Gemini or Qwen requests across samples, but concurrent calls can contend for server and network resources.
