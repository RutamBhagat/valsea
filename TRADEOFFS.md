...
- The server calls Qwen through the Modal SDK instead of its HTTP endpoint. This removes the custom HTTP request contract and limits invocation to deployed Modal methods, but the server credentials have workspace access rather than endpoint-only proxy access.
