...
CREATE TABLE `comparison_run` (
	...
	`createdAt` integer DEFAULT (cast(unixepoch('subsecond') * 1000 as integer)) NOT NULL,
	FOREIGN KEY (`audioId`) REFERENCES `audio`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `provider_run_comparison_run_id_provider_unique` ON `provider_run` (`comparisonRunId`,`provider`);